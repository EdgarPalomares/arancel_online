...
class mod extends Conexion{
     ...
     function insertar()
     {
          ...
          {
               $this->Ejecutar("insert into solicitud_arancel(nro_solicitud,fecha,user_id) values('".$this->getnro_solicitud()."','".$this->getfecha()."','".$this->getuser_id()."');");
               $boolean=1;
          }
          return $boolean;
         }
     ...
     function busqueda()
     {
          $result=$this->EjecutarSql("select * from (select `solicitud_arancel`.`id` AS `id`,`solicitud_arancel`.`nro_solicitud` AS `nro_solicitud`,`solicitud_arancel`.`fecha` AS `fecha`,`solicitud_arancel`.`user_id` AS `user_id` from `solicitud_arancel`) as bg where  bg.nro_solicitud like '%".$this->getnro_solicitud()."%'  and  bg.fecha like '%".$this->getfecha()."%'  ".$this->getorder()." ;");
     return $result;
}

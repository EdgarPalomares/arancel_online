...
///////////////configuraciones mensajes validaciones////////////////
      $n_nro_solicitud='Ingrese nro_solicitud';
      $n_fecha='Ingrese fecha';
      $n_user_id='Ingrese user_id';
...
class ctrl{
   ...
   function validaciones($aFormValues)
   {
      ...
      global $n_fecha;
      global $n_user_id;
           if ((trim($aFormValues['nro_solicitud']) == "") and ($bError ==1))
         {
            $this->objResponse->assign("mensaje","innerHTML",$n_nro_solicitud);
            $this->limpiarmensaje();
            $bError = 0;
         }
...
if($_SESSION["seleccionar"])
 ...
 $result=new Rs($this->modelo->busqueda());
         $str="
<thead><tr bgcolor='white' ><th width='5%'>#</th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'nro_solicitud')\"    >$ng_nro_solicitud<img id='imnro_solicitud'></th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'fecha')\"    >$ng_fecha<img id='imfecha'></th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'user_id')\"    >$ng_user_id<img id='imuser_id'></th></tr></thead>";
         $i=0;
      $str.="<tbody>";   
     while($result->Registros())
         {
           $i++;
     $str.="<tr ><td>".$i."</td> <td  >".$result->getCampo('nro_solicitud')."</td> <td  >".$result->getCampo('fecha')."</td> <td  >".$result->getCampo('user_id')."</td><td width='5%' onclick='xajax_ctrl.c_ver(".$result->getCampo('id').");' ><i class='fa fa-search'></i></td><td width='5%' onclick='xajax_ctrl.c_actualizar(".$result->getCampo('id').");' ><i class='fa fa-edit'></i></td><td width='5%' onclick='el(".$result->getCampo('id').");'  ><i class='fa fa-trash-o'></i></td></tr>";
         }

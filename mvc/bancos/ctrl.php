...
class ctrl{
   ...
   function iload($aFormValues)
   {
     ...
     function permisos()
     {
          ...
          $_SESSION["modulo_permisos"]='bancos';
          $_SESSION["order"]='';
   ...
   function limpiarformulario()
   {
     ...
     $this->objResponse->assign("banco","value","");
   ...
   function validaciones($aFormValues)
   {
           $bError =1;
           global $n_banco;
           if ((trim($aFormValues['banco']) == "") and ($bError ==1))
         {
            $this->objResponse->assign("mensaje","innerHTML",$n_banco);
            $this->limpiarmensaje();
            $bError = 0;
         }
...
if($_SESSION["seleccionar"])
 ...
 $result=new Rs($this->modelo->busqueda());
         $str="
<thead><tr bgcolor='white' ><th width='5%'>#</th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'banco')\"    >$ng_banco<img id='imbanco'></th></tr></thead>";
         $i=0;
      $str.="<tbody>";   
     while($result->Registros())
         {
           $i++;
     $str.="<tr ><td>".$i."</td> <td  >".$result->getCampo('banco')."</td><td width='5%' onclick='xajax_ctrl.c_ver(".$result->getCampo('id').");' ><i class='fa fa-search'></i></td><td width='5%' onclick='xajax_ctrl.c_actualizar(".$result->getCampo('id').");' ><i class='fa fa-edit'></i></td><td width='5%' onclick='el(".$result->getCampo('id').");'  ><i class='fa fa-trash-o'></i></td></tr>";
         }
...
permitida...");
    ...
    function c_actualizar($id)
   {
              ...
              $this->objResponse->assign("form","action","bancos.php");
...
");
   ...
   {
               $this->c_f_ver($id);
              $this->objResponse->assign("form","action","bancos_v.php");
...
");
   ...
   function c_eliminar_ver($id)
   {
	 ...
	 $this->objResponse->redirect('bancos',2);
     }
     else
     {
          $this->objResponse->assign("mensaje","innerHTML","Operaci&oacute;n No permitida...");

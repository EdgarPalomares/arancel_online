...
///////////////configuraciones mensajes validaciones////////////////
      ...
      $n_id_estatus_solicitud='Ingrese id_estatus_solicitud';
/////////////encabezado resultado busqueda grid/////////////////
      $ng_nro_solicitud='nro_solicitud';
      $ng_fecha_solicitud='fecha_solicitud';
      $ng_user_id='user_id';
      $ng_fecha='fecha';
      $ng_estatus='estatus';
/////////////encabezado resultado busqueda grid/////////////////
...
class ctrl{
   ...
   {
         $this->permisos();
     $this->c_id_solicitud_arancel($aFormValues['h_id_solicitud_arancel']);
     $this->c_id_estatus_solicitud($aFormValues['h_id_estatus_solicitud']);
     ...
     function permisos()
     {
          ...
          $_SESSION["modulo_permisos"]='solicitud_arancel_estatus';
          $_SESSION["order"]='';
   ...
   function limpiarformulario()
   {
     ...
     $this->objResponse->assign("id_estatus_solicitud","value","");
   ...
   function validaciones($aFormValues)
   {
      ...
      if ((trim($aFormValues['id_estatus_solicitud']) == "") and ($bError ==1))
         {
            $this->objResponse->assign("mensaje","innerHTML",$n_id_estatus_solicitud);
            $this->limpiarmensaje();
            $bError = 0;
         }
   ...
   function guardarformulario($aFormValues)
   {
      ...
      if($this->validaciones($aFormValues))
      {
          ...
          $this->modelo->setid_estatus_solicitud($aFormValues['id_estatus_solicitud']);
...
if($_SESSION["seleccionar"])
     {
          ...
          $this->modelo->setfecha_solicitud($aFormValues['fecha_solicitud']);
 ...
 $result=new Rs($this->modelo->busqueda());
         $str="
<thead><tr bgcolor='white' ><th width='5%'>#</th> <th width='20%' onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'nro_solicitud')\"    >$ng_nro_solicitud<img id='imnro_solicitud'></th> <th width='15%' onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'fecha_solicitud')\"    >$ng_fecha_solicitud<img id='imfecha_solicitud'></th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'user_id')\"    >$ng_user_id<img id='imuser_id'></th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'fecha')\"    >$ng_fecha<img id='imfecha'></th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'estatus')\"    >$ng_estatus<img id='imestatus'></th></tr></thead>";
         $i=0;
      $str.="<tbody>";   
     while($result->Registros())
         {
           $i++;
     $str.="<tr ><td>".$i."</td> <td  >".$result->getCampo('nro_solicitud')."</td> <td width='25%' >".$result->getCampo('fecha_solicitud')."</td> <td  >".$result->getCampo('user_id')."</td> <td width='25%' >".$result->getCampo('fecha')."</td> <td width='20%' >".$result->getCampo('estatus')."</td><td width='5%' onclick='xajax_ctrl.c_ver(".$result->getCampo('id').");' ><i class='fa fa-search'></i></td><td width='5%' onclick='xajax_ctrl.c_actualizar(".$result->getCampo('id').");' ><i class='fa fa-edit'></i></td><td width='5%' onclick='el(".$result->getCampo('id').");'  ><i class='fa fa-trash-o'></i></td></tr>";
         }
...
permitida...");
    ...
    function c_actualizar($id)
   {
              ...
              $this->objResponse->assign("form","action","solicitud_arancel_estatus.php");
...
<input type='hidden' name='id_estatus_solicitud' value='".$result->getCampo('id_estatus_solicitud')."'/>
");
   ...
   {
               $this->c_f_ver($id);
              $this->objResponse->assign("form","action","solicitud_arancel_estatus_v.php");
...
<input type='hidden' name='fecha_solicitud' value='".$result->getCampo('fecha_solicitud')."'/>
...
");
   ...
   function c_eliminar_ver($id)
   {
	 ...
	 $this->objResponse->redirect('solicitud_arancel_estatus',2);
     }
     else
     {
          $this->objResponse->assign("mensaje","innerHTML","Operaci&oacute;n No permitida...");
...
function sel_id_solicitud_arancel()
   ...
   function c_id_estatus_solicitud($id)
   {
      $result=new Rs($this->modelo->m_id_estatus_solicitud());
         $str="<select id='id_estatus_solicitud' class='form-control' name='id_estatus_solicitud' onchange=\"xajax_ctrl.sel_id_estatus_solicitud(xajax.getFormValues('";
     $str.="form'));\"><option value=''>Seleccione</option>";
         while($result->Registros())
          {
            		$str.="<option value='".$result->getCampo('id')."'";
    ...
    $str.=">".$result->getCampo('descripcion')."</option>";
          ...
          $str.="</select>";
          $this->objResponse->assign("d_id_estatus_solicitud","innerHTML",$str);
           return $this->objResponse;

       }
function sel_id_estatus_solicitud()
     {
      return $this->objResponse;
     }

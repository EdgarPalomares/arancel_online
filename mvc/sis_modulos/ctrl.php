...
class ctrl{
   ...
   function limpiarformulario()
   {
     ...
     $this->objResponse->assign("modulo","value","");
   ...
   function validaciones($aFormValues)
   {
      ...
      global $n_descripcion;
           if ((trim($aFormValues['modulo']) == "") and ($bError ==1))
         {
            $this->objResponse->assign("mensaje","innerHTML",$n_modulo);
            $this->limpiarmensaje();
            $bError = 0;
         }
   ...
   function guardarformulario($aFormValues)
   {
      ...
      if($this->validaciones($aFormValues))
      {
          ...
          $this->modelo->setmodulo($aFormValues['modulo']);
...
if($_SESSION["seleccionar"])
 ...
 $result=new Rs($this->modelo->busqueda());
         $str="
<thead><tr bgcolor='white' ><th width='5%'>#</th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'modulo')\"    >$ng_modulo<img id='immodulo'></th> <th onclick=\"xajax_ctrl.c_order(xajax.getFormValues('form'),'descripcion')\"    >$ng_descripcion<img id='imdescripcion'></th></tr></thead>";
         $i=0;
      $str.="<tbody>";   
     while($result->Registros())
         {
           $i++;
     $str.="<tr ><td>".$i."</td> <td  >".$result->getCampo('modulo')."</td> <td  >".$result->getCampo('descripcion')."</td><td width='5%' onclick='xajax_ctrl.c_ver(".$result->getCampo('id').");' ><i class='fa fa-search'></i></td><td width='5%' onclick='xajax_ctrl.c_actualizar(".$result->getCampo('id').");' ><i class='fa fa-edit'></i></td><td width='5%' onclick='el(".$result->getCampo('id').");'  ><i class='fa fa-trash-o'></i></td></tr>";
         }
...
permitida...");
    ...
    function c_actualizar($id)
   {
              ...
              $this->objResponse->assign("form","action","sis_modulos.php");
...
<input type='hidden' name='modulo' value='".$result->getCampo('modulo')."'/>
...
");
   ...
   {
               $this->c_f_ver($id);
              $this->objResponse->assign("form","action","sis_modulos_v.php");
...
");
   ...
   function c_eliminar_ver($id)
   {
	 ...
	 $this->objResponse->redirect('sis_modulos',2);
     }
     else
     {
          $this->objResponse->assign("mensaje","innerHTML","Operaci&oacute;n No permitida...");

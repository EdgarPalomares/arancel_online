...
class mod extends Conexion{
 ...
 function accesos()
     ...
     function insertar()
     {
          ...
          {
               $result2=new Rs($this->buscar_vst_estudiantes_pyscde());
               if (($result2->getRowCount())==1)
               {
                    $this->Ejecutar("insert into sis_usuarios(id_grupos,cedula,apellidos,nombres,login,telefono,password) values(".$this->getid_grupos().",'".$this->getcedula()."','".$this->getapellidos()."','".$this->getnombres()."','".$this->getlogin()."','".$this->gettelefono()."','".$this->getpassword()."');");
                    $boolean=1;
               }
          }
          return $boolean;
     ...
     function actualizar()
     {
          ...
          if (($result->getRowCount())==0) 
          {
               $this->Ejecutar("update sis_usuarios set cedula='".$this->getcedula()."',apellidos='".$this->getapellidos()."',nombres='".$this->getnombres()."',login='".$this->getlogin()."',correo='".$this->getcorreo()."' where id=".$this->getid().";");
               $boolean=1;
          }
          else
           {
 ...
 function buscar_vst_estudiantes_pyscde()
     {
